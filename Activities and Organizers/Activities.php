...
if(file_exists($fnc)){
	...
	if(isset($_GET["ed_keyword"])){//searchbar delete fnc
		...
		$sql = "SELECT * FROM activity_list WHERE activity LIKE '%".$keyword."%' ORDER BY activity ";
		$query=mysqli_query($con,$sql);
		
		$act_name = "None";$act_date = "None";$act_location = "None";
		$act_org = "None";$act_staff = "None";
		$is_readonly=true;$is_final=false;
		...
		if(mysqli_num_rows($query)>0){//needs to be more than 0
			...
			$is_readonly=false;
			$act_fin = $row["is_final"];
			
			if($act_fin=="Final"){$is_final=true;}
		...
		$content = edit($con,$act_name,$act_date,$act_location,$act_org,$act_staff,null,null,$act_id,false,$is_readonly,$is_final);
	...
	elseif(isset($_POST["edit_act_id"])&&isset($_POST["edit_act_name"])&&isset($_POST["edit_act_date"])){
		...
		if(isset($_POST["edit_act_fin"])){
			$act_fin = filter_var($_POST["edit_act_fin"],FILTER_SANITIZE_SPECIAL_CHARS);
			if($act_fin!="Final"){$act_fin="Tentative";$is_final=false;}
		}
		else{$act_fin="Tentative";$is_final=false;}
		...
		$query=mysqli_query($con,$sql);
			...
			if(mysqli_num_rows($query)>0&&strlen($act_name)>0&&strlen($act_date)>0){//means one activity with this info exist and has a valid name and date
				//adding phase
					...
					$sql = "UPDATE activity_list SET activity='".$act_name."', date='".$act_date."', location='".$act_location."', organizer_id='".$act_org."', staff_id='".$act_staff."', is_final='".$act_fin."' 
					WHERE id='".$act_id."'";
					$query=mysqli_query($con,$sql);
					
					$sql = "SELECT * FROM activity_list WHERE id='".$act_id."' AND activity='".$act_name."' AND date='".$act_date."' AND location='".$act_location."' AND organizer_id='".$act_org."' AND staff_id='".$act_staff."' AND is_final='".$act_fin."'";
					$query=mysqli_query($con,$sql);
					...
					if(mysqli_num_rows($query)>0){//check for success
						$msg = '	
							...
							<strong>Is final :</strong> '.$act_fin.'<br />
						...
						$is_error=false;
						$content=edit($con,$act_name,$act_date,$act_location,$act_org,$act_staff,$msg,$is_error,$act_id,true,true,$is_final);
					}else{
						$msg = 'All inputs are <strong><em>technically</em></strong> valid but the activity was not added for some reason,this might be due to an invalid activity date or the record was changed before you do so please try again. If the problem still occurs, please contact the system administrator to fix this';
						$is_error=true;
						$content=edit($con,$act_name,$act_date,$act_location,$act_org,$act_staff,$msg,$is_error,$act_id,true,false,$is_final);
					}
	...
	elseif(isset($_GET["s_act_name"])){
		...
		$sql = "SELECT * FROM activity_list WHERE activity LIKE '%".$act_name."%' ORDER BY activity";
		$query = mysqli_query($con,$sql);
		...
		if(mysqli_num_rows($query)>0){
			...
			$act_staff = $row["staff_id"];
			$act_fin = $row["is_final"];
			$content=search($act_name,$act_date,$act_location,$act_org,$act_staff,null,null,false,$act_fin);
		}else{
	...
	elseif(isset($_GET["delete"])){//searchbar delete fnc
		...
		$act_name = "None";$act_date = "None";$act_location = "None";
		$act_org = "None";$act_staff = "None";$act_fin = "None";
		...
		$content = del($act_name,$act_date,$act_location,$act_org,$act_staff,null,null,$act_id,false,$act_fin);
	...
	elseif(isset($_POST["del_act_id"])&&isset($_POST["del_act_name"])&&isset($_POST["del_act_date"])&&isset($_POST["del_act_location"])&&isset($_POST["del_act_org"])&&isset($_POST["del_act_staff"])){
		...
		$act_fin="Tentative";
		if(isset($_POST["del_act_fin"])){
			$act_fin = filter_var($_POST["del_act_fin"],FILTER_SANITIZE_SPECIAL_CHARS);
			if($act_fin!="Final"){$act_fin="Tentative";}
		}
		...
		if($val){//means valid id, email, and contact
			...
			$sql = "SELECT * FROM activity_list	WHERE id='".$act_id."' AND activity='".$act_name."' AND date='".$act_date."' AND location='".$act_location."' AND is_final='".$act_fin."'";
			$query=mysqli_query($con,$sql);
		
			if(mysqli_num_rows($query)>0){
				$sql = "DELETE FROM activity_list WHERE id='".$act_id."' AND activity='".$act_name."' AND date='".$act_date."' AND location='".$act_location."' AND is_final='".$act_fin."'";
				$query=mysqli_query($con,$sql);
				...
				if(mysqli_num_rows($query)==0){
					...
					$content = del($act_name,$act_date,$act_location,$act_org,$act_staff,$msg,false,$act_id,true,$act_fin);
				}
				else{
					...
					$content = del($act_name,$act_date,$act_location,$act_org,$act_staff,$msg,true,$act_id,true,$act_fin);
				}
			}
			else{
				$msg='The record about that organizer doesn&apos;t exist in the first place so there is nothing to delete';
				//$msg=$act_org."::".$act_staff;
				$content = del($act_name,$act_date,$act_location,$act_org,$act_staff,$msg,true,$act_id,true,$act_fin);
			}		
		}else{
			$msg = '	
	...
	elseif(isset($_POST["add_act_name"])&&isset($_POST["add_act_date"])){
		...
		if(isset($_POST["add_act_fin"])){
			$act_fin = filter_var($_POST["add_act_fin"],FILTER_SANITIZE_SPECIAL_CHARS);
			$is_final=true;
			if($act_fin!="Final"){$act_fin="Tentative";$is_final=false;}
		}
		else{$act_fin="Tentative";$is_final=false;}
		...
		*/
		...
		$query=mysqli_query($con,$sql);
			...
			if(mysqli_num_rows($query)==0&&strlen($act_name)>0&&strlen($act_date)>0){//means no activity name with this info is here
				//adding phase
					$sql = "INSERT INTO activity_list (id, activity, date, location, organizer_id, staff_id,is_final) 
					VALUES (NULL, '".$act_name."', '".$act_date."', '".$act_location."', '".$act_org."', '".$act_staff."', '".$act_fin."' )";
					$query=mysqli_query($con,$sql);
					
					$sql = "SELECT * FROM activity_list WHERE activity='".$act_name."' AND date='".$act_date."' 
					AND location='".$act_location."' AND organizer_id='".$act_org."' AND staff_id='".$act_staff."' AND is_final='".$act_fin."'" ;
					$query=mysqli_query($con,$sql);
					...
					if(mysqli_num_rows($query)>0){//check for success
						$msg = '	
							...
							<strong>Staff involved :</strong> <br />'.$act_staff.'
						';
						$content=add($con,null,null,null,null,null,$msg,false,$is_final);
					}else{
						$msg = 'All inputs are <strong><em>technically</em></strong> valid but the activity was not added for some reason,this might be due to an invalid activity date so please try again. If the problem still occurs, please contact the system administrator to fix this';
						$content=add($con,$act_name,$act_date,$act_location,$act_org,$act_staff,$msg,true,$is_final);
					}
					...
					elseif($action==="search"){
						if(isset($_GET["act_name"])){
							...
							if(mysqli_num_rows($query)>0){
								...
								$act_staff = $row["staff_id"];
								$act_fin = $row["is_final"];
								$content=search($act_name,$act_date,$act_location,$act_org,$act_staff,null,null,true,$act_fin);
							}else{
								$act_name="No record was found, try for more keywords";
								$content=search($act_name,$act_date,$act_location,$act_org,$act_staff,null,null,true,$act_fin);
							}
						}else{
							$content = search(null,null,null,null,null,null,null,true,null);
						}
						$valid_act=true;
					}
					elseif($action==="edit"){$content = edit($con,null,null,null,null,null,null,null,null,true,true,false);$valid_act=true;}
					elseif($action==="delete"){$content = del(null,null,null,null,null,null,null,null,true,null);$valid_act=true;}
					elseif($action==="add"){$content = add($con,null,null,null,null,null,null,null,false);$valid_act=true;}
...
function search($act_name,$act_date,$act_location,$act_org,$act_staff,$msg,$is_error,$show_search_bar,$act_fin){
	$sms = "";
					...
					<div id="add_org_cont" class="col-lg-6">
						...
						<div id="act_search_html" >
							...
							<div class="form-group">
								...
								<input value="'.$act_fin.'" readonly class="form-control" placeholder="None">
							</div>
...
function add($con,$act_name,$act_date,$act_location,$act_org,$act_staff,$msg,$is_error,$is_final){
	$sms = "";
	...
	$is_checked="";
	if($is_final){$is_checked="checked";}
	...
	$content_h = '<div class="row">
					<div id="add_org_cont" class="col-lg-6">
						<form method="POST" action="Activities.php" role="form">
							...
							<div class="form-group">
								<label>Is final : </label>
								<input class="c_box" name="add_act_fin" type="checkbox" '.$is_checked.' value="Final" />
							</div>
							...
							<div class="form-group">
								<label>Location</label>
								<textarea name="add_act_location" class="form-control" rows="3" placeholder="i.e. Cabinet Hill, Baguio City (Optional)">'.$act_location.'</textarea>
							</div>                                        
...
function del($act_name,$act_date,$act_location,$act_org,$act_staff,$msg,$is_error,$act_id,$show_search_bar,$act_fin){
	$sms = "";
	...
	$content = '<div class="row">
					<div id="add_org_cont" class="col-lg-6">
						...
						<form method="POST" id="delete_html" action="Activities.php" role="form">
							...
							<div class="form-group">
								<label>Is final</label>
								<input name="del_act_fin" value="'.$act_fin.'" readonly class="form-control" placeholder="None">
							</div>
...
function edit($con,$act_name,$act_date,$act_location,$act_org,$act_staff,$msg,$is_error,$act_id,$show_search_bar,$is_readonly,$is_final){
	...
	$content_h = '<div class="row">
					<div id="add_org_cont" class="col-lg-6">
						...
						<form method="POST" id="edit_html" action="Activities.php" role="form">
							...
							<div class="form-group">
								<label>Is final : </label>
								<input class="c_box" name="edit_act_fin" type="checkbox" value="Final" '.$readonly.' '.$is_checked.' />
							</div>
