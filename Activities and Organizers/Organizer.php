...
if(file_exists($fnc)){
	...
	if(isset($_GET["delete"])){//searchbar
		...
		echo '
					...
					<br />
					Name :<br />
					<input name="delete_rep_pos" readonly="true" class="form-control" placeholder="Representative Position" value="'.$rep_pos.'" >
	...
	elseif(isset($_POST["del_rep_pos"])&&isset($_POST["del_org_name"])&&isset($_POST["del_rep_name"])&&isset($_POST["del_rep_contact"])&&isset($_POST["del_rep_email"])&&isset($_POST["del_rep_addr"])){//FOR EDITING THE VALUES RECEIVNG FROM SEARCH
		...
		$rep_pos = filter_var($_POST["del_rep_pos"],FILTER_SANITIZE_SPECIAL_CHARS);
		...
		$rep_contact = filter_var($_POST["del_rep_contact"], FILTER_SANITIZE_SPECIAL_CHARS);
		$rep_email = filter_var($_POST["del_rep_email"],FILTER_SANITIZE_SPECIAL_CHARS);
		}
		
		
		$sql = "SELECT org_id FROM organizer WHERE org_name='".$org_name."' AND rep_position='".$rep_pos."' AND rep_name='".$rep_name."' AND rep_contact='".$rep_contact."' AND rep_email='".$rep_email."' AND rep_address='".$rep_addr."'";
		$query = mysqli_query($con,$sql);
		...
		if(mysqli_num_rows($query)>0){
			...
			$msg=null;
			$content = delete($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,null,$org_id);
		}
		else{
			$org_id=-1;
			$msg = '
			...
			$content = delete($con,null,null,null,null,null,null,$msg,true,$org_id);
		}
	...
	elseif(isset($_POST["delete_rep_pos"])&&isset($_POST["delete_id"])&&isset($_POST["delete_org_name"])&&isset($_POST["delete_rep_name"])&&isset($_POST["delete_rep_contact"])&&isset($_POST["delete_rep_email"])&&isset($_POST["delete_rep_addr"])){//FOR EDITING THE VALUES
		...
		$rep_pos = filter_var($_POST["delete_rep_pos"],FILTER_SANITIZE_SPECIAL_CHARS);			
		$rep_contact = filter_var($_POST["delete_rep_contact"], FILTER_SANITIZE_SPECIAL_CHARS);
		...
		if($val){//means valid id, email, and contact
			...
			$sql = "SELECT * FROM organizer	WHERE org_id='".$org_id."' AND org_name='".$org_name."' AND rep_position='".$rep_pos."' AND rep_name='".$rep_name."' AND rep_contact='".$rep_contact."' AND rep_email='".$rep_email."' AND rep_address='".$rep_addr."'";
			$query=mysqli_query($con,$sql);
		
			if(mysqli_num_rows($query)>0){
				$sql = "DELETE FROM organizer WHERE org_id='".$org_id."' AND org_name='".$org_name."' AND rep_position='".$rep_pos."' AND rep_name='".$rep_name."' AND rep_contact='".$rep_contact."' AND rep_email='".$rep_email."' AND rep_address='".$rep_addr."'";
				$query=mysqli_query($con,$sql);
				...
				if(mysqli_num_rows($query)==0){
					...
					$content = delete($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,false,$org_id);
				}
				else{
					...
					$content = delete($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,true,$org_id);
				}
			}
			else{
	...
	elseif(isset($_GET["edit"])){
		...
		echo '
			<div style="display:none;" >
					...
					<label>Organizer Name</label>
					<input name="edit_org_name" '.$readonly.' class="form-control" placeholder="Organizer Name" value="'.$org_name.'" required />
					...
					<br />
					Name :<br />
					<input name="edit_rep_pos" '.$readonly.' class="form-control" placeholder="Representative&apos;s Position" value="'.$rep_pos.'" />

					<br />
					Contact Number : [can add multiple contact, separate using comma i.e. 09099730701,(047)811-1365 ]<br />
					<input name="edit_rep_contact" '.$readonly.' class="form-control" placeholder="Contact Number" value="'.$rep_contact.'" required />
	...
	elseif(isset($_POST["ed_rep_pos"])&&isset($_POST["ed_org_name"])&&isset($_POST["ed_rep_name"])&&isset($_POST["ed_rep_contact"])&&isset($_POST["ed_rep_email"])&&isset($_POST["ed_rep_addr"])){//FOR EDITING THE VALUES RECEIVNG FROM SEARCH
		//just sanitize them before passing for security reasons
		//note : org_id is not in the post due to security reasons
		...
		$rep_pos = filter_var($_POST["ed_rep_pos"],FILTER_SANITIZE_SPECIAL_CHARS);;
		...
		$rep_contact = filter_var($_POST["ed_rep_contact"], FILTER_SANITIZE_SPECIAL_CHARS);
		$rep_email = filter_var($_POST["ed_rep_email"],FILTER_SANITIZE_SPECIAL_CHARS);
		}
		
		
		$sql = "SELECT org_id FROM organizer WHERE org_name='".$org_name."' AND rep_name='".$rep_name."' AND rep_position='".$rep_pos."' AND rep_contact='".$rep_contact."' AND rep_email='".$rep_email."' AND rep_address='".$rep_addr."'";
		$query = mysqli_query($con,$sql);
		...
		if(mysqli_num_rows($query)>0){
			...
			//$msg='<h4>The organizer&apos;s information was successfully loaded</h4>';
			$msg=null;
			$content = edit($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,null,$org_id,false);
		}
		else{
			$org_id=-1;
			$msg = '
			...
			';
			//$msg  =$org_name.":".$rep_name.":".$rep_contact.":".$rep_email .":".$rep_addr;
			$content = edit($con,null,null,null,null,null,null,$msg,true,$org_id,true);
		}
	...
	elseif(isset($_POST["edit_rep_pos"])&&isset($_POST["edit_id"])&&isset($_POST["edit_org_name"])&&isset($_POST["edit_rep_name"])&&isset($_POST["edit_rep_contact"])&&isset($_POST["edit_rep_email"])&&isset($_POST["edit_rep_addr"])){//FOR EDITING THE VALUES
		...
		$rep_contact = filter_var($_POST["edit_rep_contact"], FILTER_SANITIZE_SPECIAL_CHARS);
		...
		$rep_pos = filter_var($_POST["edit_rep_pos"],FILTER_SANITIZE_SPECIAL_CHARS);
		
		if(strlen($rep_name)==0){$rep_name="None";}
		if(strlen($rep_email)==0){$rep_email="None";}
		if(strlen($rep_addr)==0){$rep_addr="None";}
		if(strlen($rep_pos)==0){$rep_pos="None";}

		$val = true;
		//requires name and contact
		if(strlen($org_name)==0){$org_name="";$val=false;}
		if(strlen($rep_contact)==0){$rep_contact="";$val=false;}
		...
		$query = mysqli_query($con,$sql);
			...
			if(mysqli_num_rows($query)>0){//record exist and can be edit
				...
				$sql = "UPDATE organizer SET org_name='".$org_name."', rep_name='".$rep_name."', rep_position='".$rep_pos."', rep_contact='".$rep_contact."', rep_email='".$rep_email."', rep_address='".$rep_addr."' 
				WHERE org_id='".$org_id."'";
				
				$query = mysqli_query($con,$sql);
				
				$sql = "SELECT * FROM organizer WHERE org_id='".$org_id."' AND org_name='".$org_name."'	AND rep_name='".$rep_name."' AND rep_position='".$rep_pos."' AND rep_contact='".$rep_contact."' AND	rep_email='".$rep_email."' AND rep_address='".$rep_addr."'";
				...
				if(mysqli_num_rows($query)>0){//success editing
					...
					';
					$content = edit($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,false,$org_id,true);
				}
				else{
					$msg = '	
						All inputs where valid but somehow the changes are not saved, please contact the system administrator to fix this
					';
					$content = edit($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,true,$org_id,false);
				}
			...
			$msg = '	
					The organizer&apos;s record you entered contains invalid input<br />
							This might be : 
							<ul>
								<li>Organizer&apos;s name (Required)</li>
								<li>Representative&apos;s contact number (Required)</li>
	...
	elseif(isset($_GET["search"])){//for search bar case
		...
		$i=0;
			...
			echo	'<tr class="gradeX">
					...
					<td>
						<form method="POST" action="Organizer.php" >
							...
							<input class="hide" name="ed_rep_pos" type="text" value="'.$row["rep_position"].'" />
						...
						<form method="POST" action="Organizer.php" >
							...
							<input class="hide" name="del_rep_pos" type="text" value="'.$row["rep_position"].'" />
			...
			echo	'<tr class="gradeX">
					<td colspan="7" style="text-align:center;" > Total entries displayed : '.$i.'</td>
					</tr>';
		}
	...
	elseif(isset($_POST["org_name"])&&isset($_POST["rep_contact"])){
		$org_name = filter_var($_POST["org_name"],FILTER_SANITIZE_SPECIAL_CHARS);
		$rep_contact = filter_var($_POST["rep_contact"], FILTER_SANITIZE_SPECIAL_CHARS); //FILTER_SANITIZE_NUMBER_INT
		
		$rep_name="None";$rep_email="None";$rep_addr="None";$rep_pos="None";
		...
		if(isset($_POST["rep_addr"])&&strlen($_POST["rep_addr"])>0){$rep_addr = filter_var($_POST["rep_addr"],FILTER_SANITIZE_SPECIAL_CHARS);}
		
		if(isset($_POST["rep_pos"])&&strlen($_POST["rep_pos"])>0){$rep_pos = filter_var($_POST["rep_pos"],FILTER_SANITIZE_SPECIAL_CHARS);}
		...
		if(mysqli_num_rows($query)==0){//this means this new org is not already in the db
			...
			if(checker($rep_contact,$rep_email)&&strlen($org_name)){//valid email and contact //error checking
				...
				//adding record
				$sql = "INSERT INTO organizer (org_id, org_name, rep_name, rep_position, rep_contact, rep_email, rep_address) VALUES (NULL, '".$org_name."', '".$rep_name."', '".$rep_pos."', '".$rep_contact."', '".$rep_email."', '".$rep_addr."')";
				$query=mysqli_query($con,$sql);
			
				//verifying if the record is added
				$sql = "SELECT * FROM organizer WHERE org_name='".$org_name."' AND rep_name='".$rep_name."' AND rep_position='".$rep_pos."' AND rep_contact='".$rep_contact."' AND rep_email='".$rep_email."' AND rep_address='".$rep_addr."'";
				$query=mysqli_query($con,$sql);
				
				if(mysqli_num_rows($query)>0){

					$msg = '	
							The organizer&s record with this information is added to the database : <br /><br />
							<strong>Organizer Name :</strong> '.$org_name.'<br />
							<strong>Representative&apos;s Name :</strong> '.$rep_name.'<br />
							<strong>Representative&apos;s Position :</strong> '.$rep_pos.'<br />
							<strong>Representative&apos;s Contact Number :</strong> '.$rep_contact.'<br />
							<strong>Representative&apos;s Email Address :</strong> '.$rep_email.'<br />
							<strong>Representative&apos;s Adress :</strong>'.$rep_addr.'
						';
						
					$content = add($con,null,null,null,null,null,null,$msg,false);
				}else{
					$msg = '	
					...
					$content = add($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,true);
				}
			}else{
				$msg = '	
							The organizer&apos;s record you entered contains invalid input<br />
		...
		$valid_act = false;
			...
			if($act_true){
				...
				elseif($action==="edit"){$content = edit($con,null,null,null,null,null,null,null,null,null,true);$valid_act=true;}
				elseif($action==="delete"){$content = delete($con,null,null,null,null,null,null,null,null,null,true);$valid_act=true;}
				elseif($action==="add"){$content = add($con,null,null,null,null,null,null,null,null);$valid_act=true;}
...
function search($con){
	$content_h='
			...
			<br style="clear:both;" />
			<div style="overflow:hidden;overflow-x:scroll;" id="dataTables-example_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
			<table style="width:100%;max-width:100%;" class="table table-striped table-bordered table-hover" id="dataTables-example">
				<thead>
					<tr>
						<th>Organizer Name</th>
						<th>Representative&apos;s Name</th>
						<th>Representative&apos;s Position</th>
	...
	$data = "";
		...
		elseif(mysqli_num_rows($query)==0){
			$data = '<tr class="gradeX">
					<td colspan="7" style="text-align:center;" > No results found, there are no organizations registered to the database </td>
					</tr>';
		}elseif(mysqli_num_rows($query)>$limit){
			$data = '<tr class="gradeX">
...
function edit($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,$is_error,$org_id,$readonly){
	//note: prefix : ed_
	$sms = "";
			...
			<div class="row">
				<div id="add_org_cont" class="col-lg-6">
					...
					<form role="form" >
						...
						<input class="form-control" onkeyup="edit(this.value)" style="width:100%;" class="form-control input-sm" placeholder="Type the organizer name to select the record to edit" aria-controls="dataTables-example" type="search" />
					</form>
					...
					<form id="edit_html" role="form" method="POST" action="Organizer.php" >
						...
						<label>Organizer Name</label>
						<input name="edit_org_name" '.$read.' class="form-control" placeholder="Organizer Name" value="'.$org_name.'" required />
						...
						<br />
						Position :<br />
						<input name="edit_rep_pos" '.$read.' class="form-control" placeholder="Representative&apos;s Position" value="'.$rep_pos.'" />

						
						<br />
						Contact Number : [can add multiple contact, separate using comma i.e. 09099730701,(047)811-1365 ]<br />
						<input name="edit_rep_contact" '.$read.' class="form-control" placeholder="Contact Number" value="'.$rep_contact.'" required />
...
function delete($con,$org_name,$rep_name,$rep_pos,$rep_contact,$rep_email,$rep_addr,$msg,$is_error,$org_id){
			...
			<div class="row">
				<div id="add_org_cont" class="col-lg-6">
					...
					<form id="delete_html" role="form" method="POST" action="Organizer.php" >
						...
						<br />
						Position :<br />
						<input name="delete_rep_pos" readonly="true" class="form-control" placeholder="Representative&apos;s Position" value="'.$rep_pos.'" >
...
function add($con,$org_name,$rep_pos,$rep_name,$rep_contact,$rep_email,$rep_addr,$msg,$is_error){
	$sms = "";
					...
					<div id="add_org_cont" class="col-lg-6">
						<form id="add_org" role="form" method="POST" action="Organizer.php" >
							...
							<div>
								...
								<label>Organizer Name</label>
								<input name="org_name" class="form-control" type="text" placeholder="Enter the organizer&apos;s name (Required)" value="'.$org_name.'" required />
								...
								<br />
									Name : <br />
									<input name="rep_name" class="form-control" type="text" placeholder="Enter the name of representative (Optional)" value="'.$rep_name.'" />
								<br />
									Position : <br />
									<input name="rep_pos" class="form-control" type="text" placeholder="Enter the representative&apos;s position (Optional)" value="'.$rep_pos.'" />
								<br />
									Contact Number: [can add multiple contact, separate using comma i.e. 09099730701,(047)811-1365 ]<br />
									<input name="rep_contact" class="form-control" type="text" placeholder="i.e. 09099730701 or (047)811-1365 (Required)" value="'.$rep_contact.'" required />
								<br />
									Email : <br />
...
function checker($rep_contact,$rep_email){
	$val = true;
	$sep_contact = explode(",",$rep_contact);
	//09099730701 or (047)811-1365
	$contact="";
	foreach($sep_contact as $contact){
	
		if($contact!="None"){
			if(strlen($contact)==11){//mobile
				$contact_array = str_split($contact);
				foreach($contact_array as $digit){
					if(!is_numeric($digit)){
						$val = false;
						break;
					}
				...
				//rc might not be necessary
				$rc = str_replace("&#40;","(a",$contact);
				$rc = str_replace("&#41;","a)",$rc);
				$rc = str_replace("&#45;","a-a",$rc);
				
				//(047)811-1365
				$x = 0; //for error checking
				if(strlen($rc)==13){
					for($i=0;$i<strlen($rc);$i++){
						if($i==0){if($rc[$i]!='('){$x=1;$val=false;break;}} //&#40; for (
						elseif($i==4){if($rc[$i]!=')'){$x=1;$val=false;break;}}  //&#41; for )
						elseif($i==8){if($rc[$i]!='-'){$x=1;$val=false;break;}}  //&#45; for -
						else{
							if(!is_numeric($rc[$i])){$x=1;
								$val=false;break;
							}
						}
						//echo $rc[$i]."::".$x."<br />";
